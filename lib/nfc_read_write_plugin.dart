...
class NfcReadWritePlugin {
  ...
  MethodChannel("nfc_read_write");
  /// 声明监听回调通道
  static const EventChannel _eventChannel = EventChannel("nfc_read_write/event");
  ...
  static Future<dynamic> writeBlock(int blockIndex, String data,{bool encrypt=false}) async {
  ...
  static Future<dynamic>  readAll({bool decrypt=false})async {
    final dynamic data = await _channel.invokeMethod('readAll',{"decrypt":decrypt});
    return data;
  }

  static Future<dynamic> readSector(int sectorIndex,  {bool decrypt=false}) async {
    Map<String,dynamic>map ={};
  ...
  static Future<dynamic> readBlock(int blockIndex,  {bool decrypt=false}) async {
    Map<String,dynamic>map ={};
    map["blockIndex"] = blockIndex;
    map["decrypt"] = decrypt;
